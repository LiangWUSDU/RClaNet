...
weights_path = "new_weights/MA/ResNet/"
weights_file = open('new_weights/MA/ResNet/weights.txt')
weights_strings = weights_file.readlines()
## 输出文件
out_OASIS3_file = 'new_predicts/ResNet/MA/OASIS3.xls'
out_AIBL_file = 'new_predicts/ResNet/MA/AIBL.xls'
out_ADNI_file = 'new_predicts/ResNet/MA/ADNI.xls'
##
for i in range(0,len(weights_strings)):
	...
	for id in range(len(ADNI_strings)):
		...
		ADNI_risk = nib.load(ADNI_risk_file + st2).get_fdata()
	...
	for id in range(len(OASIS3_strings)):
		...
		OASIS3_risk = nib.load(OASIS3_risk_file + st1).get_fdata()
	...
	for id in range(len(AIBL_strings)):
		...
		AIBL_risk = nib.load(AIBL_risk_file + st2).get_fdata()
