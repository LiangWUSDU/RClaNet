...
#model.load_weights('new_weights/MA/CNN/weights.04-0.0670.hdf5',by_name=True)
##VGG
#from networks.VGG import VGG16
#model = VGG16((160,192,160,1))
#model.load_weights('new_weights/MA/VGG/weights.18-0.1644.hdf5',by_name=True)
## ResNet
#from networks.ResNet import Resnet
#model =  Resnet((160,192,160,1))
#model.load_weights('new_weights/MA/ResNet/weights.10-0.0565.hdf5',by_name=True)
##GoogleNet
#from networks.GoogleNet import Googlenet
#model = Googlenet((160,192,160,1))
#model.load_weights('new_weights/MA/GoogleNet/weights.05-0.2358.hdf5',by_name=True)
##MHCNN
from networks.my_model import my_model_single
model = my_model_single((160,192,160,1))
model.load_weights('new_weights/MA/MHCNN/weights.07-0.0524.hdf5',by_name=True)
import json
import xlwt
# 设置Excel编码
...
for id in range(len(ADNI_strings)):
	st1 = ADNI_strings[id].strip()
	ADNI_risk = nib.load(ADNI_risk_file + st1).get_data()
	ADNI_risk = bn(ADNI_risk)
	ADNI_risk = ADNI_risk[np.newaxis, ..., np.newaxis]
	pred1 = model.predict(ADNI_risk)
