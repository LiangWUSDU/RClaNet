...
from networks.my_model import my_model3_single
from functions.loss import focal_loss
...
def find_position(x,value):
    ...
    p = np.array(p)
    return p
weight_dir = "weights/OASIS3/att_risk_three/"
...
MCI_num = len(p_M)
AD_num = len(p_A)
epoch = (CN_num + MCI_num + AD_num)/batch_size
myGene = my_gen31(train_risk_file,train_txt,train_CDR,batch_size=batch_size)
##
...
test_epoch = (test_CN_num + test_MCI_num + test_AD_num)/batch_size
my_validGene = my_gen31(test_OASIS_risk_file,test_txt,test_CDR,batch_size=batch_size)

model =  my_model3_single((160,192,160,1))
#model.summary()
