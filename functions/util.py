...
from functions.ext.neuron.utils import interpn
...
def warp_volumes_by_ddf(input_, ddf):
    grid_warped = get_reference_grid(ddf.shape[1:4]) + ddf
    warped = resample_linear(tf.convert_to_tensor(input_, dtype=tf.float32), grid_warped)
