...
def find_position(x,value):
    ...
    p = np.array(p)
    return p
weight_dir = "new_weights/MA/MHCNN_kf10_10/"
...
model.summary()
model.load_weights('new_weights/MA/MHCNN_kf10_3/weights.04-0.1101.hdf5',by_name=True)
...
model.fit_generator(myGene,steps_per_epoch = epoch,epochs=5,validation_data=my_validGene,validation_steps=valid_epoch,callbacks=[model_checkpoint1,early_stop])
