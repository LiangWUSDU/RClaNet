...
OASIS3_strings,OASIS3_task = select_classification_task(OASIS3_strings,OASIS3_CDR,p_OC,p_OA)
...
AIBL_strings,AIBL_task = select_classification_task(AIBL_strings,AIBL_CDR,p_AC,p_AA)
...
ADNI_strings,ADNI_task = select_classification_task(ADNI_strings,ADNI_CDR,p_DC,p_DA)
ADNI_task[ADNI_task==2]=1
## 分类模型
##CNN
#from networks.my_model import my_model_CNN_single
#model = my_model_CNN_single((160,192,160,1))
#model.load_weights('new_weights/CA/CNN/weights.04-0.1016.hdf5',by_name=True)
##VGG
#from networks.VGG import VGG16
#model = VGG16((160,192,160,1))
#model.load_weights('new_weights/CA/VGG/weights.21-0.0668.hdf5',by_name=True)
## ResNet
#from networks.ResNet import Resnet
#model =  Resnet((160,192,160,1))
#model.load_weights('new_weights/CA/ResNet/weights.08-0.0432.hdf5',by_name=True)
##GoogleNet
#from networks.GoogleNet import Googlenet
#model = Googlenet((160,192,160,1))
#model.load_weights('new_weights/CA/GoogleNet/weights.05-0.2503.hdf5',by_name=True)
##MHCNN
from networks.my_model import my_model_single
model = my_model_single((160,192,160,1))
model.load_weights('new_weights/CA/MHCNN/weights.03-0.1875.hdf5',by_name=True)
...
for id in range(len(ADNI_strings)):
    ...
    pred2 = model.predict( ADNI_risk)
    predict2 = np.argmax(pred2, axis=-1)
    sheet1.write(id, 0, np.float(predict2[0]))
    sheet1.write(id, 1, np.float(ADNI_task[id]))
