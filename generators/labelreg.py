...
def generator_data(train_moving_image_file, st,train_moving_mask_file,sl):
    ...
    return moving_image,moving_mask,fixed_image,fixed_mask,affine0
...
def gen_patch(moving_image, moving_mask, fixed_image, fixed_mask):
...
def Random(moving_strings,moving_seg_strings,y_CDR):
    permutation = np.random.permutation(moving_seg_strings.shape[0])
...
def Random_image(moving_image_patch,moving_mask_patch,fixed_image_patch,fixed_mask_patch):
...
def labelreg_gen(train_moving_image_file,train_moving_image_txt,train_moving_mask_file,train_moving_mask_txt,batch_size,batch_patch_size):
    while True:
        ...
        for start in range(0, len(image_strings), batch_size):
            ...
            for id in range(start, end):
                ...
                moving_image, moving_mask,fixed_image, fixed_mask,affine0 = generator_data(train_moving_image_file, st,train_moving_mask_file,sl)
                moving_mask = to_categorical(moving_mask)
                fixed_mask = to_categorical(fixed_mask)
                moving_image_patch, moving_mask_patch, fixed_image_patch, fixed_mask_patch = gen_patch( moving_image, moving_mask, fixed_image, fixed_mask)
                moving_image_patch, moving_mask_patch, fixed_image_patch, fixed_mask_patch = Random_image(moving_image_patch,moving_mask_patch,fixed_image_patch,fixed_mask_patch)
                for start in range(0, len(moving_image_patch), batch_patch_size):
                    ...
                    yield ([moving_image_batch_patch, fixed_image_batch_patch,moving_mask_batch_patch,fixed_mask_batch_patch],
                          [flow,fixed_image_batch_patch,y_true])
