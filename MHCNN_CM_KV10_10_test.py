...
model.load_weights('new_weights/CM/MHCNN_kf10_1/weights.33-0.0267.hdf5',by_name=True)
...
##
weights_path = "new_weights/CM/MHCNN_kf10_10/"
weights_file = open('new_weights/CM/MHCNN_kf10_10/weights.txt')
weights_strings = weights_file.readlines()
##
from functions.write_excel import *
for i in range(0,len(weights_strings)):
    ...
    image_strings,y_CDR = Random(image_strings,train_CDR,p_C_valid,p_M_valid)
    y_CDR[y_CDR==0]=0
    y_CDR[y_CDR==1]=1
    predicts = []
    ...
    print(ADNI_metric)
    write_excel_xls_append('new_predicts/MHCNN/KV10/CM.xls', ADNI_metric)
