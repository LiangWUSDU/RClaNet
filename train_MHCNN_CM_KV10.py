...
def find_position(x,value):
    ...
    p = np.array(p)
    return p
weight_dir = "new_weights/CM/MHCNN_kf10_10/"
...
valid_epoch = (len(p_C_valid)+len(p_M_valid))/batch_size
myGene = my_gen_CM(train_risk_file,train_txt,train_CDR,p_C_train,p_M_train,batch_size=batch_size)
my_validGene = my_gen_CM(train_risk_file,train_txt,train_CDR,p_C_valid,p_M_valid,batch_size=batch_size)
##
model =  my_model_single_kf((160,192,160,1))
model.summary()
model.load_weights('new_weights/CM/MHCNN_kf10_1/weights.12-0.3324.hdf5',by_name=True)
